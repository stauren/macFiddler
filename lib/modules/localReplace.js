...
(function () {
  ...
  var deferred = require('deferred'),
    ...
    originExists = promisify(fs.exists),
  ...
  function exists(filePath, handler) { //deferred changed the way to handle promisify(fs.exists)
    return originExists(filePath)(handler, function (err) {
      //new deferred reject the promise if the first  parameter is true
  ...
  function pipeFile(filePath, res, headers, modDelay, file404) {
    var def = deferred(),
        ...
        matchFound = replaceRules.some(function (rule) {
          ...
          if (matchUrl && rule[1]) {
            ...
            buildFiles(url, modLocalReplace.conf.autoBuild).then(function (hasBuilt) {
              ...
              pipeFile(localFile, response, headers,
